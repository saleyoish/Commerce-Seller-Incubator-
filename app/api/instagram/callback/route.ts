  ...
  try {
    ...
    if (error) {
      return NextResponse.redirect(
        `${BASE_URL}/seller/platforms/instagram?error=${encodeURIComponent(errorDescription || error)}`
      );
    }

    if (!code) {
      return NextResponse.redirect(`${BASE_URL}/seller/platforms/instagram?error=No authorization code received`);
    }
    ...
    if (!tokenResponse.ok || tokenData.error) {
      ...
      return NextResponse.redirect(
        `${BASE_URL}/seller/platforms/instagram?error=${encodeURIComponent(errorMsg)}`
      );
    }
    ...
    const instagramUserId = userData.id || null;
    const instagramUserName = userData.name || null;
    ...
    if (result.error) {
      ...
      return NextResponse.redirect(
        `${BASE_URL}/seller/platforms/instagram?error=${encodeURIComponent('Failed to save connection')}`
      );
    }

    // Redirect back with success message
    return NextResponse.redirect(
      `${BASE_URL}/seller/platforms/instagram?success=Instagram connected successfully!&autoSync=true`
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : 'OAuth callback failed';
    console.error('Instagram OAuth callback error:', error);
    return NextResponse.redirect(
      `${BASE_URL}/seller/platforms/instagram?error=${encodeURIComponent(message)}`
    );
  }
}
